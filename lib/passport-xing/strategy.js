  ...
  this._oauth.get(url, token, tokenSecret, function (err, body, res) {
    ...
    try {
      ...
      if (user.birth_date && user.birth_date.year && user.birth_date.month && user.birth_date.day) {
          profile.birthday = new Date(user.birth_date.year+"-"+(user.birth_date.month < 10 ? "0"+user.birth_date.month : user.birth_date.month)+"-"+(user.birth_date.day < 10 ? "0"+user.birth_date.day : user.birth_date.day)+"T00:00:00");
      }
